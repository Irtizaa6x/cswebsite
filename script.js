...
function renderEventBanner() {
    if (!siteData.activeEvent) return;
    setTimeout(() => {
        const nameEl = document.getElementById('banner-event-name');
        const dateEl = document.getElementById('banner-event-date');
        const locEl = document.getElementById('banner-event-location');
        const statusText = document.querySelector('#banner-status .status-text');
        const ctaBtn = document.getElementById('banner-cta-button');
        
        if (nameEl) nameEl.textContent = siteData.activeEvent.name;
        if (dateEl) dateEl.textContent = siteData.activeEvent.date;
        if (locEl) locEl.textContent = siteData.activeEvent.location;
        if (statusText) {
            statusText.textContent = siteData.activeEvent.status === 'upcoming' ? 'Upcoming Event' : 'Live Now';
        }
        if (ctaBtn && siteData.activeEvent.ctaLink) {
            ctaBtn.href = siteData.activeEvent.ctaLink;
            if (siteData.activeEvent.ctaText) {
                ctaBtn.innerHTML = `${siteData.activeEvent.ctaText} <i class="fas fa-arrow-right"></i>`;
            }
        }
    }, 100);
}
...
function renderAbout() {
    ...
    if (aboutActivitiesGrid && siteData.activities) {
        aboutActivitiesGrid.innerHTML = siteData.activities.map((act, idx) => `
            <div class="activity-card" style="animation-delay: ${idx * 0.05}s">
...
function renderCommittee() {
    ...
    if (modContainer && siteData.committeeGroups.moderator) {
        modContainer.innerHTML = renderMemberCards(siteData.committeeGroups.moderator.members);
    }
    ...
    // 3. Executive Committee (tree with connectors)
    ...
    if (execContainer && siteData.committeeGroups.executiveCommittee) {
        ...
        execContainer.innerHTML = `
            <div class="exec-tree-container">
                <div class="exec-row top">${renderMemberCard(execs[0])}</div>
                <div class="exec-connector"></div>
                <div class="exec-row middle">${renderMemberCard(execs[1])}${renderMemberCard(execs[2])}</div>
                <div class="exec-connector"></div>
                <div class="exec-row bottom">${renderMemberCard(execs[3])}</div>
            </div>
        `;
    }
    ...
    // 5. Other groups (Multimedia, Executive Members)
...
function renderContactInfo() {
    ...
    if (!container || !siteData.contactInfo) return;
    
    container.innerHTML = siteData.contactInfo.map(info => `
...
function initContactForm() {
    ...
    form.addEventListener('submit', function(e) {
        ...
        if (!email || !/^[^\s@]+@([^\s@.,]+\.)+[^\s@.,]{2,}$/.test(email)) { alert('Valid email required.'); return; }
